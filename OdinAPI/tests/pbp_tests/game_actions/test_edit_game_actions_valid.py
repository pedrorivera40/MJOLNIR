...
class TestEditValidVolleyballPBP(unittest.TestCase):
    ...
    def test_edit_valid_notification(self):
        response = self.client.put('/pbp/actions',data=json.dumps(data["edit_valid_notification"]),content_type='application/json', follow_redirects=True)
    ...
    def test_edit_valid_score_to_error(self):
        response = self.client.put('/pbp/actions',data=json.dumps(data["edit_valid_score_to_error"]),content_type='application/json', follow_redirects=True)
        expected_msg = "Edit game action success."
        self.assertEqual(response.status_code, 200)
        self.assertMultiLineEqual(expected_msg, response.json["MSG"])