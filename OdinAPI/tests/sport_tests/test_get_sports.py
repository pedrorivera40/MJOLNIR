...
class TestGetAllSports(unittest.TestCase):
    ...
    def test_get_all_sports_bad(self):
        ...
        expected_msg = "Error en la solicitud. Debe proveerse un valor (rama deportiva o nombre del deporte)."
    ...
    def test_get_all_sports_by_branch(self):
        response = self.client.get('/sports', data=json.dumps(
            {"branch": "Femenino"}), content_type='application/json', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        print(response.json)

    def test_get_all_sports_by_name_good(self):
        response = self.client.get('/sports', data=json.dumps(
            {"sport_name": "Baloncesto"}), content_type='application/json', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        print(response.json)

    def test_get_all_sports_by_name_bad(self):
        response = self.client.get('/sports', data=json.dumps(
            {"sport_name": "Tiro al blanco"}), content_type='application/json', follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        print(response.json)

    def test_get_sport_by_id_good(self):
        response = self.client.get('/sports', data=json.dumps(
            {"sport_id": 7}), content_type='application/json', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        print(response.json)

    def test_get_sport_by_id_bad(self):
        response = self.client.get('/sports', data=json.dumps(
            {"sport_id": 999999}), content_type='application/json', follow_redirects=True)
